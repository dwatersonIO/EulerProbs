...
# Grid is 20 x 20
# 1 Put into a list so can read

grid_str = ["08", "02", "22", "97", "38", "15", "00", "40", "00", "75", "04", "05", "07", "78", "52", "12", "50", "77", "91", "08", "49", "49", "99", "40", "17", "81", "18", "57", "60", "87", "17", "40", "98", "43", "69", "48", "04", "56", "62", "00",
...
for word in grid_str:
    grid.append(int(word))

# Vertical

def get_total(start, end, increment):
    highest=0
    for number in range(start, end):
        first=number
        second=number+increment
        third=number+(increment * 2)
        forth=number+(increment * 3)
        total = grid[first]*grid[second]*grid[third]*grid[forth]
        if total > highest:
            highest=total
    return highest
...
def diagonal_right():
    highest=0  
    for start in range(0,340,20):
        for number in range(start, start+17):
            total = grid[number] * grid[number+21] * grid[number+42] * grid[number+63]
            if total > highest:
                highest=total
    return highest   
    print ()
...
def diagonal_left():
    highest=0  
    for start in range(3,339,20):
        for number in range(start, start+16):
            total = grid[number] * grid[number+19] * grid[number+38] * grid[number+57]
            if total > highest:
                highest=total
    return highest   
    print ()
...
# vertical
print (f"Vertical highest: {get_total(0, 339, 20)}")
print (f"Horizontal highest: {get_total(0, 397, 1)}")
print (f"Diagonal right: {diagonal_right()}")
print (f"Diagonal left: {diagonal_left()}")

# print (grid[3], grid[22], grid[41], grid[60])
# print (grid[19], grid[38], grid[57], grid[76])
# print()
# print (grid[23], grid[42], grid[61], grid[80])
# print (grid[39], grid[58], grid[77], grid[96])
# print()
# print (grid[323], grid[342], grid[361], grid[360])
# print (grid[339], grid[358], grid[377], grid[396])
